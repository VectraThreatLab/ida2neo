...
'''
#CREATE (null_sub:Function { name : \'null_sub\', address : \'0\' })
...
def MakeRestReq(sBatch, uCnt):
	req = urllib2.Request(URL, STATEMENT % (sBatch.rstrip(',')), {'Content-Type': 'application/json'})
...
uCnt = 1
for _ in Functions():
	sFunctionName =  GetFunctionName(_).replace('@','_at_').replace('?','_qm_')
...
for z in Functions():
	...
	for xref in XrefsTo(z):
		sCaller = GetFunctionName(xref.frm).replace('@','_at_').replace('?','_qm_')
